...
```{r}
library(reshape2)
scores_sido$sido_order <- factor(scores_sido$시도, 
                                 levels = scores_sido$시도[order(scores_sido$합계, decreasing = TRUE)])
scores_sido_melt <- melt(scores_sido, id.vars = "sido_order", measure.vars = 2:9, variable.name = "Indexes", value.name = "Scores")
```

```{r, fig.width = 12, fig.height = 9}
library(ggplot2)
source("theme_kr_HCR.R")
ggplot(data = scores_sido_melt,
       mapping = aes(x = sido_order, y = Scores, fill = Indexes)) +
  geom_bar(stat = "identity", position = position_stack(reverse = TRUE)) +
  theme_classic() +
  theme_kr_HCR +
  scale_fill_manual(name = "평가지표", values = indices_pal) +
  labs(x = "", y = "") +
