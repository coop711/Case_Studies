...
end_df <- prod_wage_melt %>% 
  subset(Year == 2014)
y1995_df <- prod_wage_melt %>% 
  subset(Year == 1995)
text_lab <- end_df %>% 
  `[`(, "Cumulative_Changes") %>%
...
main_title_2 <- "Productivity, Mean and Median Compensation (1973-2014)"
...
end_df_2 <- subset(prod_wage_2_melt, 
                   Year == 2014)
y2007_df <- subset(prod_wage_2_melt, 
                   Year == 2007)
(h1 <- ggplot() +
  geom_line(data = prod_wage_2_melt, 
            aes(x = Year, 
                y = Cumulative_Changes, 
                colour = Prod_or_Compensation), 
            size = 1.5, 
...
```{r, echo = TRUE, fig.width = 8, fig.height = 6}
(h3 <- h2 +
  geom_text(data = y2007_df, 
            aes(x = Year, 
                y = Cumulative_Changes - c(5, 10, 10), 
                label = var_lab_2)) +
  geom_text(data = end_df_2, 
            aes(x = Year + 2, 
                y = Cumulative_Changes, 
