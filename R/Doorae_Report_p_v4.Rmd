...
```{r, echo = FALSE, fig.width = 6, fig.height = 4.2, results = 'hide'}
...
```{r, echo = FALSE, fig.width = 6, fig.height = 4.2, fig.show = 'hide'}
...
doorae %$%
  ...
  mosaic_gg +
  theme_bw(base_family = "KoPubWorldDotum Medium") +
  labs(y = "사업아이템", x = "공동체상태") +
  scale_fill_viridis_d(name = "사업아이템") +
  ggtitle("조직형태와 공동체상태와 사업아이템") +
  theme(plot.title = element_text(hjust = 0.5, 
                                  size = 18, 
                                  family = "KoPubWorldDotum Bold"))
```
...
ggplot(data = doorae_ys, 
  ...
  annotate("text", x = 2.8, y = 0.54, 
           label = "중위수의 차이", 
           size = 3, 
           family = "KoPubWorldDotum Medium") + 
  annotate("text", x = 2.8, y = 0.46, 
           label = "약 3년", 
           size = 3, 
           family = "KoPubWorldDotum Medium")
```
...
ggplot(data = years_df, 
       mapping = aes(x = years, y = ecdf_years)) + 
  geom_path() + 
  geom_polygon(alpha = 0.3, fill = "red") + 
  theme_bw() + 
  theme(panel.background = element_rect("white"), 
        panel.border = element_rect(colour = "black", fill = NA)) +
  ggtitle("공동체상태와 업력") +
