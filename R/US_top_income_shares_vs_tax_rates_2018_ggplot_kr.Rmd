---
title: "US Top Income Share vs Tax Rates (1913 ~ 2018) : ggplot"
author: "coop711"
date: "`r Sys.Date()`"
...
library(ggplot2)
#> 데이터 불러 들이기
load("US_top_income_shares_vs_tax_rates_2018_v2.RData")
source("./theme_kr.R")
```
...
M1 <- ggplot(data = M_tbl, 
             mapping = aes(x = Year, y = Values, colour = Variables, linetype = Variables)) +
...
M3 <- M2 + 
  ...
  scale_y_continuous(name = "상위 1% 소득 점유율(%)", breaks = y1_at, labels = labels1_at, limits = c(0, 1),
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 20, margin = margin(t = 0, r = 0, b = 12, l = 0)))
M3
``` 
...
Mi_tbl <- M_tbl
Mi_tbl[Mi_tbl$Variables == "Marginal_z", "Values"] <- 1 - Mi_tbl[Mi_tbl$Variables == "Marginal_z", "Values"]
Mi1 <- ggplot(data = Mi_tbl, mapping = aes(x = Year, y = Values, colour = Variables, linetype = Variables)) +
  geom_line(show.legend = FALSE) +
...
Mi3 <- Mi2 + 
  ...
  labs(title = "상위 1% 소득점유율과 한계 소득세율") +
  annotate("text", x = c(1985, 1955), y = c(0.8, 0.35), label = c("한계 소득세율", "상위 1% 소득점유율"), 
           family = "HCR Dotum LVT", size = 6) +
  theme(plot.title = element_text(hjust = 0.5, size = 20, margin = margin(t = 0, r = 0, b = 12, l = 0)))
Mi3
``` 
...
R1 <- ggplot(data = Rates_tbl, mapping = aes(x = Year, y = Values, colour = Variables, linetype = Variables)) +
  geom_line(show.legend = FALSE) +
  geom_point(data = subset(Rates_tbl, Rates_tbl$Variables %in% c("Rate_99_z", "Rate_1_z")), 
             mapping = aes(x = Year, y = Values, shape = Variables), fill = "white", size = 3, show.legend = TRUE)
R2 <- R1 +
...
R3 <- R2 + 
  ...
  annotate("text", x = 1925, y = 0.9, label = "한계 소득세율", 
           family = "HCR Dotum LVT", size = 6) +
  theme(plot.title = element_text(family = "KoPubWorldDotum Bold", hjust = 0.5, size = 20),
...
RK1 <- ggplot(data = Rates_K_tbl, 
              mapping = aes(x = Year, y = Values, colour = Variables, linetype = Variables)) +
  geom_line(show.legend = FALSE) +
  geom_point(data = subset(Rates_K_tbl, Rates_K_tbl$Variables %in% c("Rate_99_K_z", "Rate_1_K_z")), 
             mapping = aes(x = Year, y = Values, shape = Variables), fill = "white", size = 3, show.legend = TRUE)
RK2 <- RK1 +
  scale_colour_manual(values = c("red", "black", "black"), guide = FALSE) +
  scale_linetype_manual(values = c("longdash", "solid", "solid"), guide = FALSE) +
