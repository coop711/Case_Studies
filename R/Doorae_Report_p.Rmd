...
```{r, echo = FALSE, message = FALSE}
doorae %$%
  table(.$"공동체상태", .$"조직형태", dnn = c("공동체상태", "조직형태")) %>%
# doorae$"공동체상태" %>%
#   table(doorae$"조직형태", dnn = c("공동체상태", "조직형태")) %>%
  cbind(., "계" = rowSums(.)) %>%
  rbind(., "계" = colSums(.)) %>%
  kable(caption = "ㅇ공동체상태와 조직형태(계)", align = c("c", "r", "r", "r"))
doorae %$%
  table(.$"공동체상태", .$"조직형태", dnn = c("공동체상태", "조직형태")) %>%
# doorae$"공동체상태" %>%
#   table(doorae$"조직형태", dnn = c("공동체상태", "조직형태")) %>%
...
#   table(doorae$"조직형태", dnn = c("공동체상태", "조직형태")) %>%
  ...
  rbind(., "계" = colSums(.)) %>%
  round(digits = 1) %>%
  kable(caption = "ㅇ조직형태와 공동체상태(%, 계)", align = c("c", "r", "r"))
```
...
doorae %>%
  `[`(bi_corp, v_names) %>%
  format(big.mark = ",") %>%
  kable(row.names = FALSE, col.names = c_names, align = c("c", "c", "c", "c", "r", "r"))
```
...
```{r, results='asis', echo = FALSE}
doorae %$%
  table(.$"공동체상태", .$"사업아이템", dnn = c("공동체상태", "사업아이템")) %>%
# doorae$"공동체상태" %>%
...
doorae %>%
  ...
  `[`(1:20, ) %>%
  format(big.mark = ",") %>%
  kable(row.names = FALSE, col.names = c_names_v_14, align = c("l", "l", "r"))
```
...
doorae %>%
  ...
  `[`(1:20, ) %>%
  format(big.mark = ",") %>%
  kable(row.names = FALSE, col.names = c_names_v_15, align = c("l", "l", "r"))
```
...
doorae %>%
  ...
  `[`(1:20, ) %>%
  format(big.mark = ",") %>%
  kable(row.names = FALSE, col.names = c_names_s_14, align = c("l", "l", "r"))
```
...
doorae %>%
  ...
  `[`(1:20, ) %>%
  format(big.mark = ",") %>%
  kable(row.names = FALSE, col.names = c_names_s_15, align = c("l", "l", "r"))
```
