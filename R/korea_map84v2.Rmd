...
library(broom) #> `tidy()`
library(extrafont) #> "KoPubWorldDotum Medium" => "KoPubWorldDotum Medium"
options(width = 132)
```
...
qtm(shp2_84, 
    fill = "SHAPE_AREA", 
    fill.title = "면적") +
  tm_layout(fontfamily = "KoPubWorldDotum Medium")
```
...
g1 <- g0 + geom_text(data = map2_data, 
            ...
            size = 2,
            colour = "blue") 
g2 <- g1 + theme(axis.title.x = element_text(family = "KoPubWorldDotum Medium"),
        axis.title.y = element_text(family = "KoPubWorldDotum Medium"),
        text = element_text(family = "KoPubWorldDotum Medium")) 
g3 <- g2 + labs(x = "경도", 
       y = "위도") 
g4 <- g3 + annotate("text", 
           x = Dokdo_coord$lon, 
...
#               colour = "black") +
  coord_map() +
  theme(legend.text = element_text(family = "KoPubWorldDotum Medium"),
