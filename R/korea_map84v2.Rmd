...
library(broom) #> `tidy()`
library(extrafont) #> "Malgun Gothic" => "Malgun Gothic"
options(width = 132)
```
...
Dokdo_coord <- geocode("Dokdo", source = "google") #> (Data Science Toolkit 활용)
Ulleung_coord <- geocode("울릉도", source = "google")
```
...
qtm(shp2_84, 
    fill = "SHAPE_AREA", 
    fill.title = "면적") +
  tm_layout(fontfamily = "Malgun Gothic")
```
...
g1 <- g0 + geom_text(data = map2_data, 
            ...
            size = 2,
            colour = "blue") 
g2 <- g1 + theme(axis.title.x = element_text(family = "Malgun Gothic"),
...
#               colour = "black") +
  coord_map() +
  theme(legend.text = element_text(family = "Malgun Gothic"),
...
popup2 <- paste(shp2_84$SIGUNGU_NM2, 
               "\n", 
