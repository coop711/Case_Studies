...
```{r, geom_bar, fig.width = 12, fig.height = 5}
...
# o_df <- data_23$Income.inequality %>% 
#   order %>%
#   data_full[., c("Income.inequality", "Country")]
```

```{r, fig.width = 12, fig.height = 6}
g1 <- ggplot(data = o_df, 
             mapping = aes(x = factor(Country, levels = Country),
                           y = Income.inequality)) 
g2 <- g1 + 
...
p1 <- ggplot(data = Index_inequality_df, 
             mapping = aes(x = Income_inequality, 
                           y = Index_HS,
                           label = Country))
p1.1 <- p1 + 
  geom_point(col = "red")
...
nudge_x_text <- ifelse(hjust_text == "middle", 0.01, 
                       ifelse(hjust_text == "right", -0.05, 0.05))
p6 <- p2 + 
  geom_text(hjust = hjust_text, 
            vjust = vjust_text, 
...
# p9 <- p8 + geom_abline(intercept = lm_ineq$coef[1], 
#                        slope = lm_ineq$coef[2], 
#                        colour = "blue")
p9_1 <- p8 + 
...
```{r, warning = FALSE}
ggsave("../pics/Inequality_vs_HS_Index_ggplot.png", p9_1, dpi = 75)
ggsave("../pics/Inequality_vs_HS_Index_ggplot_se.png", p9_2, dpi = 75)
```
