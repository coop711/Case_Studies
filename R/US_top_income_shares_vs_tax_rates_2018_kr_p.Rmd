---
title: "US Top Income Share vs Tax Rates (1913 ~ 2018)"
author: "coop711"
date: "`r Sys.Date()`"
...
dev.copy(png, file ="../pics/US_Top_Income_Share_vs_MTR_kr_wide.png", width = 960, height = 540)
dev.off()
``` 
...
title(main = "상위 1% 소득점유율과 최고 한계세율", xlab = "연도", cex.main = 1.8)
text(x = 1973, y = 0.35, labels = "최고 한계세율", cex = 1.2)
text(x = 1998, y = 0.17, labels = "상위 1% 소득점유율", cex = 1.2)
...
par(mar = c(5, 6, 4, 6) + 0.1,
    family = "KoPubWorldDotum Medium")
plot(Rate_99_z ~ Year, data = top_income_tax, type = "b", pch = 24, col = "black", bg = "black", axes = FALSE, ann = FALSE, xlim = c(1910, 2020), ylim = c(0, 1))
lines(Rate_1_z ~ Year, data = top_income_tax, type = "b", pch = 24, col = "black", bg = "white")
box()
axis(side = 1, at = seq(1910, 2020, by = 10), labels = seq(1910, 2020, by = 10))
y2_at <- c(0, z(Rate_99, c(100, 200, 300)), 1)
labels2_at <- format(c(min(Rate_99), c(100, 200, 300), max(Rate_99)),
                    digits = 2, nsmall = 1)
axis(side = 2, at = y2_at, labels = labels2_at, las = 1)
ylab_2 <- "평균 소득 (1913 = 100)"
# ylab_2 <- "Average Income (1913 = 100)"
mtext(ylab_2, side = 2, line = 4)
# mtext(ylab_2, side = 2)
# par(new = TRUE)
lines(Marginal_z ~ Year, data = top_income_tax, type ="l", lty = 2, col = "red", lwd = 2, xlim = c(1910, 2020), ylim = c(0, 1))
y4_at <- c(0, z(top_income_tax$Marginal, seq(20, 80, by = 20)), 1)
labels4_at <- format(c(min(top_income_tax$Marginal), seq(20, 80, by = 20), max(top_income_tax$Marginal)),
                    digits = 2, nsmall = 1)
axis(side = 4, at = y4_at, labels = labels4_at, las = 1)
ylab_4 <- "최고 한계세율(%)"
mtext(ylab_4, side = 4, line = 3)
title(main = "상위 1%와 하위 99% 소득 증가\n(자본이득 제외)", xlab = "연도", cex.main = 1.5)
legend("bottomright", legend = c("하위 99%", "상위 1%"), pch = 24, col = "black", pt.bg = c("black", "white"), inset = 0.05)
# legend("bottom", legend = c("Bottom 99%", "Top 1%"), pch = 24, col = "black", pt.bg = c("black", "white"))
text(x = 1925, y = 0.9, labels = "최고 한계세율", cex = 1.2)
...
par(mar = c(5, 6, 4, 6) + 0.1,
    family = "KoPubWorldDotum Medium")
plot(Rate_99_K_z ~ Year, data = top_income_tax, type = "b", pch = 24, col = "black", bg = "black", axes = FALSE, ann = FALSE, xlim = c(1910, 2020), ylim = c(0, 1))
lines(Rate_1_K_z ~ Year, data = top_income_tax, type = "b", pch = 24, col = "black", bg = "white")
box()
axis(side = 1, at = seq(1910, 2020, by = 10), labels = seq(1910, 2020, by = 10))
y3_at <- c(0, z(Rate_99_K, seq(100, 500, by = 100)), 1)
labels3_at <- format(c(min(Rate_99_K), seq(100, 500, by = 100), max(Rate_99_K)),
                    digits = 2, nsmall = 1)
axis(side = 2, at = y3_at, labels = labels3_at, las = 1)
mtext("평균 소득 (1913 = 100)", side = 2, line = 4)
